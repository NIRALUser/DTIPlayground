...
```
    $ dmriprep init --config-dir my/config/dir
```
...
```
    $ dmriprep update [--config-dir my/config/dir]
```
...
To run with default protocol generated by dmriprep:

```
    $ dmriprep run -i IMAGE_FILE -o OUTPUT_DIR -d [ MODULE1 MODULE2 ... ]
```
`"-d"` option (default protocol) works as described in **make-protocols** command. But you need to specify `"-d"` for the default pipeline from the template.  
