...
def command_init(args):
    ...
    user_module_dir=home_dir.parent.joinpath('modules/dmriautotract').absolute()
    ...
    source_template_path=Path(dtiplayground.dmri.tractography.__file__).parent.joinpath("templates/protocol_template.yml")
    ...
    system_module_paths = [Path(dtiplayground.__file__).resolve().parent.joinpath('dmri/tractography/modules')]
...
def check_if_module_exists(args,module_name):
    ...
    system_module_root_dir = Path(dtiplayground.__file__).resolve().parent.joinpath("dmri/tractography/modules")
...
def command_add_module(args):
    ...
    if base_module_name is None: ### NEW module 
        ...
        module_template_fn =Path(dtiplayground.__file__).resolve().parent.joinpath("dmri/tractography/templates/module_template.py")
...
def command_make_protocols(args):
    ...
    proto=dtiplayground.dmri.tractography.protocols.Protocols(options['config_dir'],global_vars=options['global_variables'])
...
def get_args():
    ...
    version=info['dmriautotract']['version']
    logger("VERSION : {}".format(str(version)))
    config_dir=Path(os.environ.get('HOME')).joinpath('.niral-dti/dmriautotract-'+str(version))
    # ## read template
...
import dtiplayground.dmri.tractography
import dtiplayground.dmri.tractography.modules
import dtiplayground.dmri.tractography.protocols
