...
def interlace_compute(image_obj):
    ...
    for gidx in range(0,g):
        ...
        logger("\rGradient {}/{}, Corr: {:.4f}, Max translation : {:.4f}px, Max angle : {:.4f} degree".format(gidx,g,corr,max_norm,max_angle_in_deg))
...
def interlace_check(image_obj, computation_result,
    ...
    for idx,g in enumerate(computation_result):
        #logger("Checking Gradient {}/{} ".format(idx,result_size-1),dtiprep.Color.PROCESS)
        result=g
        ...
        if not excluded:
            color=dtiprep.Color.OK
        else:
            color=dtiprep.Color.WARNING
            excluding_gradients_indexes.append(idx)
        logger("[Grad.idx:{0:03d}, Org.idx:{1:03d}]\tBaseline: {2}, Max.Trans: {3:.2f}px, Max.Rotation: {4:.2f}deg, Corr: {5:.4f}, Corr-Z: {6:4f}"
