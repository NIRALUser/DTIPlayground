...
def build_atlas(args):
    ...
    logger("--------------- Atlasbulder begins ----------------------",dmri.common.Color.INFO)
    bldr=AtlasBuilder()
...
def initialize_logger(logpath, no_log_timestamp, no_verbosity):
    ## default log setting
    dmri.common.logger.setLogfile(logpath)
    dmri.common.logger.setTimestamp(not no_log_timestamp)
    dmri.common.logger.setVerbosity(not no_verbosity)
